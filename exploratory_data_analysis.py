...
def statisticians(a, b):    
    return {
        # Correlation between a & b
        "corrcoef"   : np.corrcoef(a, b),  
        # Standar desviation between a & b     
        "std"        : np.std(a),                                  
        "std_sispi"  : np.std(a),                                              
        "std_GPM" : np.std(a),                               
        "var"        : np.var(a),                                  
        ...
        "var_GPM" : np.var(a),                           
        "cov"        : np.cov(a),  
        # Absolute error between a & b
        "ae"         : abs(a-b),  
        # Mean square error between a & b   
        #"mse"        : abs(a- b),
...
def files_with_nan_values_in_dataset():
    files_with_nan_values = []
    files = fileslist(config['DIRS']['DATASET_HABANA'])
    
    for file in files:
        data = read_serialize_file(file)
        if np.isnan(data["Q2"]).any() or np.isnan(data["T2"]).any() or np.isnan(data["RAIN_SISPI"]).any() or np.isnan(data["RAIN_GPM"]).any():
            files_with_nan_values.append(file.split('/')[-1])

    if len(files_with_nan_values) > 0:
        write_serialize_file(files_with_nan_values, "outputs/files_with_nan_values_in_dataset.dat")


if __name__ == "__main__":
    min_max_values_in_dataset()
    files_with_nan_values_in_dataset()
